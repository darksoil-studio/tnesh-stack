...
import { GetonlyMap, HoloHashMap, MemoHoloHashMap } from "./holo-hash-map.js";
...
/**
 * Map the given MemoHoloHashMap's values with the given mapping function
 */
export function mapMemoValues<K extends HoloHash, V, U>(
  map: MemoHoloHashMap<K, V>,
  mappingFn: (value: V, key: K) => U
): MemoHoloHashMap<K, U> {
  return new MemoHoloHashMap((key) => {
